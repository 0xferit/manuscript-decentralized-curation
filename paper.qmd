...
6. The protocol computes the weighted mean $\mu$ and weighted standard deviation $\sigma$ of submitted scores. Let $W = \sum_i w_i$ denote the total round weight:
...
$$
s_i = \lfloor p_i \cdot w_i \rfloor, \qquad r_i = w_i - s_i.
$$

The protocol slashes $s_i$ tokens and returns the remainder $r_i$. This graduated slashing replaces binary slashing: near-boundary deviations incur small losses, while extreme deviations incur total loss. If $\sigma < \varepsilon_\sigma$, step 7 is skipped entirely: $p_i = 0$ for all curators, $s_i = 0$, and all locked tokens are released from finalization.
