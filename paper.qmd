...
With equal attack-phase duration, the Trojan strategy does not help colluders. Final colluder stake share *decreases* with longer delay (0.148 at $T = 0$ vs 0.100 at $T = 100$), and mean error also falls (0.104 vs 0.080). Honest-phase stake accumulation exists but is more than offset by the additional rounds of stake redistribution that dilute the colluding bloc's position. At $\phi = 0.15$ under these parameters, delayed-onset collusion is strictly worse for the attacker than attacking immediately. Testing whether this result holds at higher collusion fractions or with adaptive bias strategies that ramp gradually rather than switching abruptly is a natural extension.
