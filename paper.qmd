...
- `RetractPending`: the author has initiated retraction. A cooldown period runs during which challenges are still accepted.
- `Retracted`: the author removed the bond after the cooldown; the item remains visible historically with frozen confidence.
...
- **Authors** submit claim blobs, post author bonds, and may later retract or amend claims.
...
2. The item enters `Live` status immediately and remains challengeable while bonded.
...
6. If the challenge succeeds, the item is `Debunked` and the bond is transferred according to the payout rules. If the challenge fails, the item returns to `Live` status.
...
Each pool has its own reward budget. The pool creator or any sponsor can top it up. Relevance-round reward floors are paid from that local budget, not from a vague global treasury. If the budget runs dry, new relevance rounds pause; the canonical record remains challengeable.
...
| Stale claims linger forever | Confidence only accumulates while bonded; retracted claims remain historical rather than active |
