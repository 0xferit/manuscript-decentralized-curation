...
The coherence game requires curators to independently report relevance scores in $[0, 1]$. Under commit-reveal, curators cannot observe others' votes before committing. Each curator $i$ has a private signal $\theta_i$ about the policy-implied relevance $r$ and forms a belief about the distribution of others' signals. Here $r$ denotes the latent relevance value implied by the pool policy, $\theta_i$ denotes curator $i$'s noisy private estimate of that value, $K$ is the coherence-threshold multiplier from the slashing rule, and $\phi^*$ denotes an approximate region of visibly increasing relevance distortion in the reported collusion sweeps. It is a descriptive simulation marker, not an equilibrium or incentive-compatibility threshold.

**Claim 3.** *A sufficiently specific policy, concentrated honest signals, and limited colluding stake can support policy-centered coordination. These conditions do not establish that reporting one's private signal is a Bayesian Nash equilibrium. The full reward, slashing, and appeal rules must make truthful reporting a best response, and equilibrium existence would still not establish which equilibrium participants select [@kong2016peerprediction].*

A counterexample follows directly from the specified low-dispersion reward rule. Consider 15 equal-weight curators who all observe $r=0.5$ without noise, with $K=1.25$, $\varepsilon_\sigma=\sigma_{\mathrm{ref}}=0.02$, $\rho=0.1$, $R=0.01$ ETH, and an appeal mean-difference threshold of $0.01$. These are defaults or permitted pool settings. All reports of $0.5$ give each curator $0.000066667$ ETH. One report of $0.51$ instead gives $\mu=0.5006667$ and $\sigma\approx0.00249444$, still below the distance-slashing floor, and increases that curator's reward to approximately $0.000141500$ ETH. An appeal returning the policy-implied mean of $0.5$ cannot exceed the configured difference threshold. With only one base reward funded and no replenishment, neither outcome leaves enough budget for another round, so future round rewards do not offset this gain. Commit-reveal does not prevent this preplanned unilateral deviation. Accurate signals and absence of collusion therefore do not suffice for truthful best response.
...
Relevance-round escalation adds incentives to anticipate appeal committees as well as the initial committee. It is intended to make policy-faithful judgment a focal point, but that equilibrium-selection effect is unproved. As in the relevance-layer description, a larger committee can improve precision when it adds unbiased independent signals of comparable quality and sufficiently disperses voting weight; increased committee size alone does not establish lower variance. This requires more distinct observations: additional seats assigned to the same curator do not supply independent signals. An appeal overturns the first score only if the mean difference exceeds the configured threshold; original-round penalties then depend on the specified appeal rule. Small profitable deviations can survive that threshold, as the counterexample shows. Whether escalation raises the colluding stake needed for a given amount of distortion remains an open quantitative question because escalation dynamics are not simulated here. Majority-stake capture and shared bias are not removed by larger committees. Competitive exit could supply an additional economic deterrent only if users recognize the degradation, funded alternatives exist, and migration reduces the attacker's expected returns; these conditions remain unvalidated. A possible refinement is to ask curators for a prediction of dispersion and score it with a proper scoring rule. Whether such a prediction distinguishes informed agreement from lazy convergence or resists collusion remains an open question.
...
The preceding collusion sweeps fix $\sigma_{\text{honest}} = 0.08$. The observed distortion region $\phi^*$ depends on the gap between honest noise and colluder deviation: at low $\sigma_{\text{honest}}$, a +0.30 bias stands out sharply; at high $\sigma_{\text{honest}}$, honest disagreement overlaps with colluder bias, making collusion harder to distinguish. To characterize this dependence, we repeat the full E2'-Adv collusion sweep at $\sigma_{\text{honest}} \in \{0.05, 0.08, 0.10, 0.12, 0.15, 0.20\}$.
