...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. If $\sigma < \sigma_{\min}$, the round is degenerate: the mean is accepted as a valid relevance score, and coherence slashing is skipped (the band has collapsed). Round rewards are reduced by a pool-configurable factor $\rho \in [0, 1]$ rather than withheld entirely. Zero reward would create a variance-injection incentive: curators would score slightly off their true signal to push $\sigma$ above the floor, producing systematic bias. Reduced reward taxes lazy convergence (the lazy strategy earns $\rho R$ per degenerate round instead of full $R$) while preserving honest curators' income on genuine-consensus claims. Lazy convergence and genuine consensus are structurally indistinguishable at the single-round level; the defense against lazy strategies is the lifetime expected value: reduced degenerate-round rewards combined with appeal tail risk (a visibly wrong lazy score can be appealed and overturned, slashing the lazy curators) make the lazy strategy dominated over repeated participation. $\sigma_{\min}$ can be computed from the pool's rolling historical $\sigma$ distribution (e.g., the 10th percentile of recent rounds) rather than a fixed constant, adapting the degeneracy floor to the pool's natural variance without governance.
