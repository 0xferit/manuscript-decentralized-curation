...
![Relevance round sequence. Curators stake into a pool, are drafted proportional to stake, commit/reveal scores, and face coherence-based slashing. Weight capping limits whale influence; near-flat rounds are discarded. Dissenting curators may appeal to a larger committee at escalating stakes.](diagrams/fig-relevance-round.png){#fig-relevance-round}
...
The theoretical structure of the escape deserves scrutiny. For single-peaked preferences on a single dimension, the generalized median mechanism is strategyproof: no individual voter can benefit from misreporting [@moulin1980strategy]. The coherence game operates on the same single dimension (relevance in $[0, 1]$). If curators' preferences are single-peaked around their true signal, the comparison to Moulin's result applies. However, the aggregation rule is the stake-weighted mean, not the median. The mean is manipulable: any individual curator can shift it by reporting an extreme value. The $K \sigma$ coherence band partially compensates by slashing reports far from the mean, making unilateral extreme-value manipulation costly. But the band does not prevent a coordinated coalition from shifting the mean itself: if enough curators agree on a biased target, the mean moves toward the target and the coalition stays inside the band while honest reporters may fall outside it. This is exactly the failure mode E2-Adv characterizes. The formal escape from George's impossibility (continuous scalars, not ranked alternatives) coexists with a distinct manipulability gap: the weighted mean is vulnerable to coalition coordination at lower stake fractions than a median-based mechanism would require. The design does not claim to eliminate strategic manipulation; it claims to narrow the attack surface to coordinated coalitions exceeding a measurable stake threshold $\phi^*$.

Relevance-round escalation changes the equilibrium analysis. Without escalation, curators converge on "what the current committee will vote," which is vulnerable to lazy or coordinated majorities that exceed $\phi^*$ within the committee. With escalation, the focal point shifts to "what would survive appeal by a larger committee." By the Condorcet jury theorem, a larger committee with independent signals produces a mean closer to the policy-implied true relevance than a smaller one. A dissenting minority that believes the first-round score is wrong can appeal; if the appeal committee agrees with the minority, the original majority is slashed. The threat of this outcome disciplines first-round curators to score carefully rather than lazily. The effective $\phi^*$ rises because a colluding bloc must be large enough to survive not only the current round but also a potential appeal round with a larger committee. The formal conditions under which escalation shifts the equilibrium are a design argument grounded in the Condorcet analogy, not a proven theorem; simulation of escalation dynamics is future work.
...
| Colluding curators try to bend relevance | Drafted committees, stake-only round weight, weight caps, slashing, relevance-round escalation |
