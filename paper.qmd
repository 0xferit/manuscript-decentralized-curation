...
**Token-curated registries.** TCRs propose coordinating on list membership through stake, challenge, and voting primitives [@goldin2017tcr]. Asgaonkar and Krishnamachari develop the closest formal game-theoretic analysis of this mechanism class, deriving conditions under which challenges occur and identifying multi-equilibrium regions [@asgaonkar2018tcrgame]; the equilibrium claims in the Equilibrium Analysis section operate in the same spirit, though under a mechanism that separates accuracy from relevance rather than compressing admission into a single vote. Wang and Krishnamachari analyze the engagement failure of deployed TCRs and propose a token-inflation remedy [@wang2018tcrinflation]; the bootstrapping failure observed in Truth Post is architecturally different because disputes route to Kleros rather than to a native token electorate, but their analysis is the nearest prior framing of the engagement-failure family Truth Post sits in. Ito and Tanaka propose a centrality-based alternative to random curator assignment using PageRank over a citation graph [@ito2019tcrcitation]; the drafted-curator design here uses stake-weighted random drafting, which avoids the pre-existing-topology requirement at the cost of weaker competence targeting. Kosmarski and Gordiychuk propose a TCR for scholarly journal curation [@kosmarski2020tcrjournal]; their instantiation retains the single-dimension compression Truth Post avoids, but the application pattern (TCR repurposed for an editorial context) is the nearest prior art. Crites, Maller, Meiklejohn, and Mercer give the first provably secure TCR construction with cryptographic vote privacy [@crites2020reputable]; "trustless" in this paper is cryptoeconomic (no single point of operational control, bonded incentives, decentralized dispute resolution), not the cryptographic-secrecy sense their construction targets. Truth Post differs from the TCR line overall by treating the unit as claims plus evidence plus disputes, and by explicitly separating accuracy from relevance instead of compressing everything into one registry.

**Peer prediction and decentralized peer review.** Peer-prediction schemes elicit truthful reports through scoring that rewards agreement with others' reports conditional on one's own [@miller2005peerprediction; @prelec2004bts]. The commit-reveal coherence game here shares the same incentive pattern but anchors the focal point to a public policy document rather than to elicited-belief structure. Beyond news, systematic surveys of blockchain-based scientific peer review [@favaro2024peerreview] and recent open-science peer-review mechanisms [@finke2024peerreview] operate in an adjacent framework family (remuneration, disclosure, reputation, certificates) and are relevant comparison points for cross-domain instantiation of the four-step framework; their domain differs but their decomposition of quality into separable dimensions is consistent with the approach taken here.
