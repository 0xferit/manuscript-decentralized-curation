...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. Let $v_i$ denote curator $i$'s revealed score and let $K$ denote the coherence-threshold multiplier. Curators inside the coherence band ($|v_i - \mu| \le K\sigma$) are not penalized. Curators outside the band lose a fraction of their locked tokens that scales linearly with distance:

$$
p_i = \min\!\Bigl(1,\;\frac{|v_i - \mu|/\sigma - K}{K}\Bigr)
$$
