...
The manipulation vectors that remain open in the continuous-scalar setting are: (a) off-chain focal-point coordination, where a coalition agrees on a biased target score before the commit phase; (b) Sybil-assisted stake concentration, where a single actor splits stake across identities to exceed the per-identity weight cap; and (c) policy-ambiguity exploitation, where curators exploit vague rubric language to justify extreme scores that survive the coherence band. Vector (a) is the empirically characterized failure mode: E2-Adv shows that a coordinating bloc exceeding approximately 15-20% of stake can shift the weighted mean far enough to punish honest reporters and preserve its own stake. The formal escape from George's impossibility therefore coexists with a demonstrated empirical failure mode at moderate collusion levels. The design does not claim to eliminate strategic manipulation; it claims to narrow the attack surface to coordinated coalitions above a measurable threshold.
