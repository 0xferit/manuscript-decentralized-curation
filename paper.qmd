...
The framework is designed to be domain-agnostic: the four steps are not specific to any single domain, and what changes across domains is the quality decomposition and the mechanism mix. Step 3 constrains that mechanism mix, not the admissibility of a domain. Each mechanism family carries its own precondition: challenge-based accuracy requires that falsification be feasible and cheaper than exhaustive verification, while the coherence game requires a relevance policy specific enough to discipline scoring. A domain can satisfy one and fail the other. Where claims are narrative and not adjudicable, the accuracy layer is unavailable while relevance scoring against a published policy remains intact; where a pool's policy is too vague to discipline scoring, the converse holds. Failing step 3 therefore removes a layer, not the domain. This paper fully instantiates one domain (news) and sketches a second (advertising). The architectural pattern is intended to generalize, but demonstrated generality requires additional instantiations beyond those presented here.
...
Popper's core point remains operationally useful: a claim is meaningful for scientific inquiry when it exposes itself to potential refutation [@popper1959logic]. The same asymmetry bounds the accuracy mechanism, and only that one. If a claim is trivially verifiable, challenge adds nothing, because anyone can check it directly without a bond to make checking worthwhile; the claim may still require relevance curation. If a claim is not falsifiable at all, challenge cannot adjudicate its truth, but the protocol does not discard it: the `NonFalsifiable` reason below makes falsifiability itself the challengeable property. Relevance is scored against a published policy rather than adjudicated as true or false, so neither bound applies to it.
...
*(i) The curation policy is specific enough that all competent curators' private signals $\theta_i$ are distributed around the policy-implied relevance $r$ with bounded variance.*
...
| $\sigma_{\text{honest}}$ | 0.08 (swept 0.05--0.20) | Standard deviation of honest curator scores | Assumed moderate noise | Unvalidated; swept |
...
The defense against bad policy is competitive selection, not governance. The analogy is to newspapers and online communities, where founding a publication is permissionless and a poorly curated one loses its readers. That analogy carries a precondition this paper has already questioned: the lemons argument above is precisely that quality is not legible before consumption, and selection cannot run on a signal nobody can read. What differs at the pool level is that the protocol publishes the signal. Pool policies, challenge histories, adjudication outcomes, and accumulated confidence are on-chain and comparable across pools, so selection operates on observable protocol state rather than on unaided quality perception. The protocol does not need to prevent bad pools; it needs to make pool creation cheap, switching costs low, and pool behavior legible enough that competitive selection can function. The blueprint identifies residual switching frictions (reputation lock-in, funder coordination, round-cycle capital commitment) that may slow this dynamic in practice. Interfaces still need to decide which pools to feature, warn about, or ignore, and cross-pool aggregation remains an interface concern by design rather than a protocol responsibility.
