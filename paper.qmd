...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. Rounds with $\sigma < \sigma_{\min}$ (pool-configurable floor, $\sigma_{\min} > 0$) are cancelled as degenerate rather than accepted as meaningful curation; the slashing formula below is not applied in that case.
