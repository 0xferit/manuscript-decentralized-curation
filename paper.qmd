...
4. When drafted, the protocol draws seats from each curator proportional to their stake. Each seat locks a fixed token amount $L$ (pool-configurable). A curator with staked capital $s_i$ is eligible for at most $\lfloor s_i / L \rfloor$ seats; curators with $s_i < L$ are not eligible for drafting. Let $n_i$ denote the number of seats drawn for curator $i$. The curator's round weight $w_i$ equals their total locked tokens:
