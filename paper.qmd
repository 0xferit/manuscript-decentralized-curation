...
3. Each seat in the lottery corresponds to $L$ locked tokens (pool-configurable, $L > 0$). Let $s_i$ denote curator $i$'s staked capital. Only curators with $s_i \ge L$ are eligible; the draft weight $d_i$ is the number of seat-tickets the curator holds:
...
Locked tokens are simultaneously the curator's influence on the weighted mean and their maximum loss. Unlocked tokens ($s_i - n_i \cdot L$) remain freely movable.
...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. Rounds with $\sigma \le \sigma_{\min}$ (pool-configurable floor, $\sigma_{\min} > 0$) are cancelled as degenerate rather than accepted as meaningful curation; the slashing formula below is not applied in that case.
