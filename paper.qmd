...
| Curators converge on conventional wisdom, suppressing nuanced truth | Escalation shifts the focal point to what survives appeal; graduated slashing makes lazy convergence costly rather than free |
...
**Curator collusion.** A colluding bloc that coordinates scores can shift the weighted mean and slash honest reporters who fall outside the coherence band. Commit-reveal prevents real-time vote copying; draw-and-lock randomizes committee composition so attackers cannot pre-commit bribes to specific curators; graduated slashing raises the minimum bribe needed to cover deviation cost. Claim 3 derives the collusion threshold $\phi^*$, and E2-Adv and @sec-e2-prime characterize it empirically at approximately 0.15 to 0.20 for 15-member committees.

**Non-falsifiable claims.** A vague or unfalsifiable claim cannot be debunked via the accuracy layer because no evidence can refute it. Without a dedicated challenge reason, such claims would be forced into binary `Debunking` adjudication, where jurors lack grounds to rule against them. The `NonFalsifiable` reason routes these to adjudication on falsifiability rather than truth, and E3 quantifies the retention-cost reduction as a function of prevalence and adjudication accuracy.

**Off-topic and template-violating claims.** A claim that passes on-chain structural checks (non-empty CIDs, valid version pointers) can still violate the pool's semantic requirements: wrong topic domain or disallowed source classes. `ScopeViolation` and `TemplateViolation` challenge reasons allow post-publication enforcement of these semantic constraints without requiring computationally expensive on-chain content analysis at submission time.
...
**Stale claims.** Confidence accumulates only while a claim's bond is active. An author who withdraws freezes confidence permanently; an author who leaves an outdated claim bonded risks challenge as new evidence emerges. The incentive to withdraw obsolete claims is direct: capital locked on a stale claim earns no new value and faces growing challenge risk as the claim ages past its relevance window.
