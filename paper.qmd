...
$$
\delta_i = \lfloor p_i \cdot w_i \rfloor, \qquad r_i = w_i - \delta_i.
$$

The protocol slashes $\delta_i$ tokens and returns the remainder $r_i$. This graduated slashing replaces binary slashing: near-boundary deviations incur small losses, while extreme deviations incur total loss. If $\sigma < \varepsilon_\sigma$, step 7 is skipped entirely: $p_i = 0$ for all curators, $\delta_i = 0$, and all locked tokens are released from finalization.
