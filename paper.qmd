...
This paper argues that many hard problems decompose into a curate-then-coordinate structure, that the difficulty is concentrated in the curation phase rather than the coordination phase, and proposes a general framework for trustless decentralized curation. The framework has four steps: identify the hidden curation problem behind an apparent downstream failure; identify the relevant quality dimensions; require falsifiability where the mechanism depends on challenge; and design distinct mechanisms for each quality dimension instead of collapsing all quality into one score. We instantiate the framework for news, where two dimensions dominate: **accuracy**, which is global and contestable, and **relevance**, which is local and non-binary.

The protocol design reflects that decomposition. Accuracy is handled through bonded publication, open challenges, and external decentralized dispute resolution. Relevance is handled through drafted curator rounds under a public policy, with coherence-based rewards and slashing. Confidence is accumulated bond-time rather than a truth label. Author reputation acts as a slow-moving non-monetary bond; curator reputation affects drafting probability but not final vote weight. Pool creation is permissionless, contracts are treated as immutable, and governance is minimized by design: we prefer no need for governance over good governance.

Truth Post, launched in 2023, partially implemented the accuracy layer only. The deployment failed to bootstrap sustained participation, but it demonstrated that the submission-and-challenge flow is deployable and directly informed the current complete design. Redesigned simulations in this paper show that debunking false claims is economically attractive under reasonable juror accuracy, that drafted-curator relevance rounds filter competence while remaining vulnerable above a concrete collusion threshold, that dedicated `NonFalsifiable` challenges materially reduce bad-item retention, and that separating author and curator reputation avoids the strongest failure mode of reputation-weighted voting while preserving entry benefits.

This paper addresses only the information-to-knowledge transformation. Coordination given shared knowledge is out of scope.
...
My thesis is blunt: **many hard problems decompose into curate-then-coordinate, and the difficulty is concentrated in the curation phase, not the coordination phase.**
...
Generative AI makes the gap wider. Writing a convincing paper, a polished essay, or functional software used to require the competence that made the output credible. AI breaks that coupling. The *form* of quality is now cheap: formatting, fluency, plausible citation patterns, respectable structure. The *substance* of quality remains expensive: correctness, coherence, domain understanding, and judgment. We are entering a regime of **information abundance and knowledge scarcity**.
...
- **A revised evaluation program aligned to the current design.** The experiments no longer assume the older reputation-weighted voting model or the older third-verdict ambiguity framing.
...
Different dimensions need different mechanisms. In the news domain, accuracy and relevance are not the same problem. Accuracy is global and contestable: a claim either withstands a debunking challenge or it does not. Relevance is local and scalar: what matters depends on the pool's public policy, scope, and current context.
...
3. Curators are drafted using a weighted lottery where the score is:

$$
d_i = s_i + \alpha \cdot \mathrm{rep}^{\mathrm{cur}}_i
$$

where $s_i$ is curator stake and $\mathrm{rep}^{\mathrm{cur}}_i$ is curator reputation.

4. Drafted curators commit and reveal relevance scores in $[0,1]$.
5. Final round weight is based on **slashable stake only**, not on reputation:
...
This design preserves the entry benefits of reputation without granting unbonded voting power. That is a major departure from the older paper version.
...
The current design separates two reputation systems:
...
- **Curator reputation** is a drafting boost only. It makes competent low-capital curators more likely to be selected, but it does not increase final vote weight inside a relevance round.

This separation is important. The older design allowed reputation to directly increase curation weight. The current design rejects that because it grants influence not fully backed by slashable capital.
...
| Reputation creates unbonded influence | Curator reputation affects drafting only; author reputation is a separate standing bond |
...
- Does separating author reputation from curator reputation preserve the intended benefits without reintroducing the old reputation-weighting failure mode?
...
E2 evaluates the current relevance design rather than the older weight-allocation model. Curators are drafted into committees from a larger pool. Reputation affects drafting probability, but final round weight comes only from slashable stake. The mechanism computes a weighted mean $\mu$, a weighted standard deviation $\sigma$, and slashes outliers beyond $K\sigma$. Near-flat rounds are cancelled rather than treated as meaningful curation.
...
## E4: Reputation after the redesign

The reputation redesign is one of the biggest differences between the current protocol and earlier drafts. Reputation is no longer a single generic quantity that directly adds to curation weight. Author reputation and curator reputation now serve different purposes.
...
### E4b: curator reputation as a drafting boost only

E4b studies whether the new curator-reputation design still helps low-capital competent curators enter the system. The experiment sweeps the curator draft-boost parameter $\alpha$ and the curator reputation decay rate $\delta$.

![E4b: Rounds until a cash-poor competent curator reaches median draft score, across curator reputation parameters.](analysis/fig/e4b_curator_entry_heatmap.png){#fig-e4b-entry}

![E4b: Relevance error under the same parameter sweep. Because curator reputation does not affect final round weight, the distortion cost remains modest.](analysis/fig/e4b_curator_error.png){#fig-e4b-error}

The new design preserves the intended benefit, but more cleanly than dramatically. With no curator reputation boost, cash-poor experts never reach median draft score within the 300-round horizon. At the stronger end of the sweep, that falls to roughly 77 rounds at $\alpha=1.0$, $\delta=0.01$, and to roughly 38 rounds at $\alpha=1.0$, $\delta=0.05$. Because reputation affects only drafting probability and not final round weight, the error cost remains materially lower than in the old model.

### E4c: curator reputation laundering after honest buildup

The obvious attack is to build curator reputation honestly and then use the extra drafting probability to coordinate a later attack. E4c models that scenario.

![E4c: Error and draft amplification under a Sybil buildup-then-attack strategy when curator reputation affects only drafting probability.](analysis/fig/e4c_curator_sybil.png){#fig-e4c}

The attack still exists, but its leverage is limited to committee selection rather than direct voting power. At $\alpha=0.5$, Sybils occupy roughly 10.8% of attack-phase committee seats against a 10% population share. That is a real advantage, but it is much weaker than the old regime in which reputation directly increased round weight inside the committee.
