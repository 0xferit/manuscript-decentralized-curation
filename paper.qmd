...
The constant-bias collusion model assumes colluders attack from round 1. A more strategic adversary might behave honestly for $T$ rounds to accumulate stake through coherence rewards, then switch to biased reporting. We test this Trojan strategy at $\phi = 0.15$ with delays $T \in \{0, 25, 50, 100\}$.

![E2'-Adv Trojan: mean relevance error (left) and final colluder stake share (right) at $\phi = 0.15$ under varying attack-onset delays. Error bars show 95% confidence intervals over 100 seeds. $T = 0$ is the constant-bias baseline.](analysis/fig/e2_prime_trojan.png){#fig-e2-prime-trojan}

Mean error over all 200 rounds *decreases* with longer delay (0.104 at $T = 0$ vs 0.093 at $T = 100$) because the attack phase is shorter. However, final colluder stake share *increases* monotonically (0.148 at $T = 0$ vs 0.164 at $T = 100$), confirming that honest-phase accumulation allows colluders to retain more stake after activation. The effect is modest at $\phi = 0.15$: a 10.8% increase in terminal colluder stake share. The mean-error metric understates the Trojan risk because it averages over the full simulation; the attack-phase error (rounds $T$--200) is concentrated in fewer rounds but may be more intense per round. Characterizing per-phase error decomposition and the interaction of Trojan delay with larger colluding fractions are natural extensions.
