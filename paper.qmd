...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. Round rewards scale linearly with $\sigma$ rather than switching at a threshold. Let $\sigma_{\mathrm{ref}}$ denote a reference dispersion level (pool-configurable; can be computed from the pool's rolling historical $\sigma$ distribution, e.g., the median of recent rounds, adapting without governance). Let $\rho \in [0, 1]$ denote the minimum reward fraction for zero-dispersion rounds. The reward factor is:

$$
f_{\mathrm{reward}} = \rho + (1 - \rho) \cdot \min\!\Bigl(1,\;\frac{\sigma}{\sigma_{\mathrm{ref}}}\Bigr)
$$

At $\sigma = 0$ the reward is $\rho R$ (minimum); at $\sigma \ge \sigma_{\mathrm{ref}}$ the reward is full $R$; between them the transition is linear. This avoids the cliff that a hard threshold would create: a step function at $\sigma_{\min}$ would incentivize curators to inject artificial variance (score 0.87 instead of 0.90) to cross the threshold, producing systematic bias. The smooth curve makes the marginal gain from small variance injection negligible. Lazy convergence and genuine consensus are structurally indistinguishable at the single-round level; the defense against lazy strategies is the lifetime expected value: low-sigma rounds pay reduced rewards and a visibly wrong lazy score can be appealed and overturned (slashing the lazy curators), making the lazy strategy dominated over repeated participation. When $\sigma$ is near zero the coherence band collapses and graduated slashing is skipped (the slashing formula divides by $\sigma$); the mean is still accepted as a valid relevance score and locked tokens are returned in full.

7. Let $v_i$ denote curator $i$'s revealed score and let $K$ denote the coherence-threshold multiplier ($K > 0$). Curators inside the coherence band ($|v_i - \mu| \le K\sigma$) are not penalized. Curators outside the band lose a fraction of their locked tokens that scales linearly with distance:

$$
p_i = \min\!\Bigl(1,\;\max\!\bigl(0,\;\frac{|v_i - \mu|/\sigma - K}{K}\bigr)\Bigr)
$$

At the band boundary the penalty is zero; at twice the boundary distance ($2K\sigma$ from the mean) the penalty is total loss of locked tokens. The protocol slashes $p_i \cdot w_i$ tokens from curator $i$'s locked amount and returns the remaining $(1 - p_i) \cdot w_i$. This graduated slashing replaces binary slashing: near-boundary deviations incur small losses, while extreme deviations incur total loss.
...
E2 evaluates the relevance layer under a simplified predecessor model: curators are drafted into committees from a larger pool using purely stake-weighted selection with the per-identity weight cap disabled. The current design replaces weight caps with draw-and-lock seat allocation and graduated slashing; E2 has not yet been updated to reflect these changes. The qualitative dynamics (competence filtering, collusion thresholds) are expected to carry over because the underlying incentive structure (stake-weighted influence, coherence-based penalties) is preserved, but simulation validation of the draw-and-lock model with graduated slashing is future work. Here $\mu$ denotes the weighted mean of committee scores, $\sigma$ denotes the weighted standard deviation, and $K$ denotes the coherence-threshold multiplier used in the slashing rule. In the E2 simulation, degenerate rounds ($\sigma < \sigma_{\min}$, implemented as `flat_round_stddev_min`) are treated as cancelled and excluded from scoring; the current design instead accepts the mean as a valid score, skips slashing, and pays linearly scaled rewards. This is a simulation-vs-design gap: E2 results reflect the cancellation model, not the smooth reward-scaling model.
