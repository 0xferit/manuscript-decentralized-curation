...
6. The protocol computes the weighted mean $\mu$ of submitted scores and the weighted standard deviation $\sigma$. If $\sigma < \sigma_{\min}$ (pool-configurable floor, $\sigma_{\min} > 0$), the round is degenerate: the mean is accepted as a valid relevance score, but coherence slashing is skipped and round rewards are withheld. Curators face no penalty and earn no reward; genuine consensus is preserved without incentivizing low-effort convergence.
