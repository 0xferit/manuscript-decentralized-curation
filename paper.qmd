...
Truth Post is not a single website. The protocol has three layers (@fig-architecture):
...
![Claim state machine. Items enter `Live` on bonded publication. Challenges route through external DDR; successful challenges lead to `Debunked` (bond transferred), failed challenges restore `Live`. Author-initiated retraction enters a cooldown (`RetractPending`) during which challenges are still accepted. Terminal states have double borders.](diagrams/fig-claim-states.png){#fig-claim-states}
...
Permissionless pool creation is the right architectural stance because it keeps bad policy local rather than protocol-global. The tradeoff is that quality control shifts outward. Interfaces still need to decide which pools to feature, warn about, or ignore, and cross-pool aggregation remains an interface concern by design rather than a protocol responsibility.
