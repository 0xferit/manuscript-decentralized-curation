...
The framework is designed to be domain-agnostic: the four steps are not specific to any single domain, and what changes across domains is the quality decomposition and the mechanism mix. Step 3 constrains that mechanism mix, not the admissibility of a domain. Challenge-based accuracy requires feasible falsification and an economic advantage over exhaustive verification; relevance curation requires a policy specific enough to support meaningful scoring. A domain can satisfy one precondition and fail the other. At the framework level, narrative claims could receive relevance curation without factual-truth adjudication, provided that instantiation defines suitable admission, lifecycle, and ranking rules. The present Truth Post design does not implement that relevance-only mode: a successful `NonFalsifiable` challenge closes the item and removes it from active feeds. Failing step 3 therefore need not exclude a domain from the framework, but it does require a different mechanism mix. This paper fully instantiates one domain (news) and sketches a second (advertising). The architectural pattern is intended to generalize, but demonstrated generality requires additional instantiations beyond those presented here.
