...
**Sybil attack cost analysis.** The per-identity weight cap of 10% means a single identity can control at most 10% of a drafted round's effective weight. However, creating additional Ethereum addresses is costless. The real Sybil deterrent is `curatorMinStakeWei`: each identity must independently meet the minimum stake to be eligible for drafting. For a whale with capital C who splits across N identities, the eligibility cost is `N * curatorMinStakeWei`, and if all N are drafted they control up to `min(N * 10%, 100%)` of round weight.

Eligibility does not guarantee drafting. The protocol drafts curators via VRF stake-weighted randomness from the full eligible set. If the attacker's total stake is a small fraction of the pool's eligible stake, most attacker identities may not be drafted in any given round. The figures below are therefore lower-bound eligibility costs, not guaranteed-control costs; actual control probability depends on the attacker's share of total eligible stake.

Concrete example: with the reference pool's target of 15 drafted curators, controlling a majority of round weight would require 6 identities (60% weight) all being drafted, and each identity's stake must be large enough to reach the 10% effective-weight cap (i.e., `s_i >= 10% * S_drafted`); minimum-stake identities in a well-capitalized pool will not reach the cap. If `curatorMinStakeWei` is 0.1 ETH, the eligibility floor is 0.6 ETH in locked capital. If `curatorMinStakeWei` is 1.0 ETH, the eligibility floor is 6.0 ETH. In a low-liquidity pool where attacker stake dominates the eligible set, draft probability approaches certainty; in a well-capitalized pool with many honest curators, it drops substantially.
...
**Quantitative deterrence conditions.** The economic deterrent against frivolous challenges depends on the challenger losing their counter-stake when the challenge fails. The paper's E1 simulation characterizes the economics under stated assumptions: at juror accuracy p=0.80, jury size N=5, and stake-to-bounty ratio S/B=0.25, a legitimate challenger's expected value is approximately +0.87 times the bounty. The deterrent for frivolous challengers is asymmetric and weaker: a suppression attacker who files a challenge they expect to lose faces expected value of approximately -0.24 times the bounty (including tax and DDR fees on the same basis as the +0.87x figure), because the counter-stake (0.25B) is much smaller than the bounty (B). The frivolous-challenge penalty is real but modest relative to the legitimate-challenge reward.

Conditional on having identified a false claim, legitimate challenges remain profitable across a wide range of juror accuracy: even at p=0.60, expected value is approximately +0.60 times the bounty (before fees), and at p=0.50, approximately +0.38 times the bounty. These figures do not include the search cost of finding a false claim; the paper's Proposition 2 analyzes the full search-and-challenge economics separately. The mechanism's economic viability degrades gradually with juror accuracy rather than collapsing at a sharp threshold.
