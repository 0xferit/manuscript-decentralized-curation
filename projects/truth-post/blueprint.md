...
Relevance-game parameters such as coherence threshold `K`, seat size `L`, dispersion floor `epsilon_sigma`, minimum reward fraction `rho`, the leak multiplier, appeal stake, appeal threshold, maximum escalation depth, and the round reward floor are currently calibration placeholders. Their intended role is clear: values that are too low weaken discipline against incoherent, concentrated, or low-effort curation, while values that are too high risk curator non-participation, excessive round cancellation, or overly conservative scoring. The main paper reports sensitivity sweeps for selected relevance parameters, but these do not establish truthful reporting as an equilibrium or calibrate the full parameter family. Its Curator voting strategy section gives a permitted-parameter example of profitable unilateral dispersion padding below the distance-slashing floor.
...
At `sigma = 0` the reward is `rho * R` (minimum). At `sigma >= sigma_ref` the reward is full `R`. Between them the transition is linear. This rule retains the reward-padding limitation described in the Relevance Round State Model and the main paper’s Curator voting strategy section; smoothing alone does not establish truthful incentives.
...
6. The threat of appeal is intended to make the expected appeal outcome a focal point for first-round curators. A larger appeal committee can improve precision around the policy-implied relevance target when it adds independent, unbiased signals of comparable quality and sufficiently disperses voting weight. Additional seats assigned to one curator do not add independent observations. This concentration argument does not establish truthful best responses: an appeal must also be worth filing, and its mean must differ enough to exceed the configured success threshold. Small profitable deviations may remain below that threshold, as illustrated in the main paper’s Curator voting strategy section.
7. Escalation can expose a whale’s original locked stake to losses if the appeal broadens independent participation enough to overturn the original score. A larger committee does not remove the underlying stake concentration or guarantee correction, even when the whale is a stake minority. A majority-stake attacker can retain expected influence as committee size grows. Deterrence through loss of pool utility additionally depends on the competitive-selection conditions in the Relevance Round State Model and on the staking asset’s value being tied to the pool; it does not cover externally motivated attackers who accept those losses.
...
- escalation: a larger appeal committee may reduce a coalition's overrepresentation from a lucky draw when it broadens independent participation; any improvement in collusion resistance also depends on appeal entry and the mean-difference threshold, and has not been established by the single-round collusion sweeps
...
- escalation: apply the conditional appeal defense in Flow F-bis; a stake minority is not by itself sufficient to ensure that an appeal is filed or succeeds
- competitive exit: rely on the conditional pool-selection hypothesis in the Relevance Round State Model; deterrence additionally requires that the attacker's loss in capital value exceed the benefit of capture, which is not guaranteed for an independently valued staking asset or an externally motivated attacker
...
Per-identity weight caps were removed as security theater: Sybil adversaries bypass them by creating cheap identities. The proposed defenses against whale concentration are escalation, graduated slashing, and the conditional competitive-exit mechanism described above.
...
The Sybil threat is therefore most acute in low-liquidity pools where attacker stake dominates the eligible set. In such pools, the attacker's identities are likely to be drafted and to represent a large share of effective weight. The seat size `L` sets a per-identity eligibility floor, but the effective defense depends on the pool's total honest-stake depth, the coalition's strategy and the viability of appeals, not on `L` alone; the main paper's simulation-derived collusion range is not a general security threshold.
...
- How should reward scaling and appeal rules change to remove the profitable dispersion-padding deviation documented in the main paper’s Curator voting strategy section while preserving genuine low-dispersion consensus? The current Flow F and Flow F-bis rules remain unchanged pending that analysis, tracked in [issue #141](https://github.com/0xferit/manuscript-decentralized-curation/issues/141).
