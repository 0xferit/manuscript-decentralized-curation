...
**Quantitative deterrence conditions.** The economic deterrent against frivolous challenges depends on the challenger losing their counter-stake when the challenge fails. The paper's E1 simulation characterizes the economics under stated assumptions: at juror accuracy p=0.80, jury size N=5, and stake-to-bounty ratio S/B=0.25, a legitimate challenger's expected value is approximately +0.87 times the bounty. Applying the same payout model in reverse (the challenger files against a true claim and expects to lose), the derived frivolous-challenge expected value is approximately -0.24 times the bounty. The asymmetry is notable: the frivolous-challenge penalty is much smaller than the legitimate-challenge reward, because the counter-stake (0.25B) is much smaller than the bounty (B). The frivolous-challenge penalty is real but modest relative to the legitimate-challenge reward.
