...
Concrete example: with the reference pool's target of 15 drafted curators, controlling a majority of round weight would require 6 identities (60% weight) all being drafted. If `curatorMinStakeWei` is 0.1 ETH, the eligibility floor is 0.6 ETH in locked capital. If `curatorMinStakeWei` is 1.0 ETH, the eligibility floor is 6.0 ETH. In a low-liquidity pool where attacker stake dominates the eligible set, draft probability approaches certainty; in a well-capitalized pool with many honest curators, it drops substantially.
