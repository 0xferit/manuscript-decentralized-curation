...
- **Not testable as stated**: "This infrastructure prevented $2M in losses" (without a specified causal comparison, identification assumptions, or supporting evidence)
...
**Anti-relitigation rule.** After a challenge resolves as `ChallengeFailed`, a follow-on challenge against the same nomination in the same round MUST present materially new evidence or a distinct unadjudicated violation. Refiling substantially the same losing case is invalid; DDR jurors dismiss it as relitigation. This bounds serial challenge griefing without capping the number of challenges, imposing deadlines, or escalating costs. The bound is proportional to the nomination's vulnerability surface: the number of independently challengeable assertions times the number of distinct evidence angles per assertion. A clean nomination with few assertions has a small surface; a complex nomination with many falsifiable claims has a larger one. Nomination templates that cap the number of required assertions provide a harder upper bound on this surface area. The rule's effectiveness also depends on DDR juror accuracy in distinguishing relitigation from genuinely novel challenges; borderline cases impose defense costs on the nominee even when ultimately dismissed. Serial challenges are further self-limiting under partial disbursement: only the challenged nomination is escrowed, and each failed challenge transfers the counter-stake to the author, compensating delay on-chain. The residual risk is externally motivated delay attacks where off-chain benefit exceeds on-chain cost; this is acknowledged but not solvable by any economic mechanism.
...
| Curator exit | No cooldown; tokens not locked in active relevance rounds and not held against finalized rounds still appealable under the paper's escalation rules are withdrawable at any time (inherited from the paper's coherence-game stake model) |
...
This profile supports documented output claims ("we built X"). A narrative such as "X prevented Y from happening", without a specified causal comparison or evidentiary test, is not testable as stated. A nomination that contains no testable assertions under the template and evidence policy is challengeable as `NonFalsifiable`. Counterfactual causal estimates are not inherently untestable; some can be identified and estimated from observed data under explicit assumptions [@dahabreh2019generalizing]. This profile does not specify standards for adjudicating those estimates.
...
A coherence game rewards agreement among independently committed reports. A specific public rubric can supply a substantive focal point: curators seek to anticipate judgments based on the same criteria. Claim 3 in the main paper discusses conditions that can support concentration around policy-implied relevance, but does not establish truthful reporting as a Bayesian Nash equilibrium under the full reward and appeal rules. Signal concentration and truthful best responses are distinct questions.
