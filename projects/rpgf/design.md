...
5. **Settlement** (nomination-specific): disputed nominations resolve via DDR. If debunked, the provisional share is redistributed as a supplementary payment to the beneficiaries referenced by surviving nominations. If the challenge fails or DDR times out, the nomination returns to holdback for a 7-day grace period; if no new challenge is filed, the share is released to the referenced beneficiary record.
