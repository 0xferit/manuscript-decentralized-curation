...
A known limitation: the mechanism resists small colluding minorities, but at our reference parameters (K=1.25, 15-member committees) it degrades when a coordinated bloc exceeds roughly colluding fraction of roughly 15 to 20 percent. The threshold depends on the coherence parameter, committee size, and policy specificity. Below that threshold, colluder impact remains limited in our simulations; above it, a coordinating bloc can bend the relevance signal. This constraint is quantified in the paper but not yet solved.
