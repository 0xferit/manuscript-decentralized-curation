...
**Defense:** The specific attack is that dispute-dependent revenue dries up in peacetime, causing curators to leave. The architectural defense is that relevance-round rewards are drawn from the pool reward budget, which is funded by pool creators and sponsors independently of dispute volume. This decouples curator compensation from dispute activity: peacetime does not mechanically reduce curator income as long as the pool is funded. Both dispute-dependent inflows (challenge tax, counter-stake share) are zero in peacetime, but these are supplementary, not primary. The remaining question (whether sponsors will fund pools long-term) is the bootstrap and sustainability problem addressed in OV-6, not the deflationary spiral specifically.
...
_Identifier note: plain numeric headings retain legacy attack IDs from the original red-team list; `OV-*` headings denote open-vulnerability entries added during later review._
