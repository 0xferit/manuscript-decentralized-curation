...
A jury can only resolve what is actually specified. If the claim is ambiguous, jurors will rationally coordinate on a *shared interpretation*—which can be wrong.
...
- If someone challenges an article, Kleros (a decentralized dispute protocol) [@kleros2024yellowpaper] selects random jurors
...
- Scope: **Global.** Pools can vary on *relevance* policy, but they cannot mint their own “accurate-for-us” labels. If a pool tries to tag falsehoods as “Accurate,” it becomes slashable via the shared dispute process (e.g., a cross-pool court like Kleros).
...
**Funding Path: Advertising as "Proof of Truth"**
- **Current State:** Ads are often "Proof of Waste": companies burn money to buy attention and signal quality indirectly.
- **Mechanism:** An advertiser makes a claim ("Our product lasts 10 years") and **stakes** on it.
...
- **Peacetime yield:** Even when political/news disputes are quiet, commercial claims create constant verification demand—keeping curators paid and active.
